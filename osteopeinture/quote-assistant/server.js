...
app.post('/api/email/standalone-draft', express.json(), async (req, res) => {
  try {
    ...
    const userPrompt = [
      ...
      `- For quote_send in French: open with "Voici la soumission" (informal: "Voici la soumission ci-jointe." / formal: "Voici la soumission ci-jointe."). Then a CTA mentioning both adjustments AND the deposit/calendar slot. Full informal FR example: "Salut [Name], Voici la soumission ci-jointe. Hésite pas si tu as des ajustements à faire. Fais-moi signe quand t'es prêt à aller de l'avant avec le dépôt pour qu'on te réserve la place dans notre calendrier. Merci, [Signer]"`,
